# Модуль 3, Урок 4: Доработка Dino 3D — Game Over, Restart, UI

**Цель урока:** собрать "игровой цикл" (start -> play -> game over -> restart), добавить UI (очки/время), завершить Dino 3D и закрепить материал модуля (мини-тесты).

---

Сейчас Dino 3D уже похож на игру, но у него нет главного: понятного финала.
Игрок должен понимать: "я проиграл", "вот мой результат", "вот как начать заново".

---

Сегодня сделаем то, что отличает "демо" от "игры": **Game Over**, **перезапуск** и **UI**.

---

## Теория 1: Что такое "состояние игры" (Game State)

Когда игра идёт, мы выполняем управление, спавн и счёт.
Когда игра окончена, это должно остановиться.

Для этого удобно держать булевую переменную:

```csharp
bool isGameOver;
```

И использовать её как "переключатель":

- если `isGameOver == true` -> показываем UI Game Over, блокируем управление
- если `isGameOver == false` -> игра работает

---

```quiz
id: l2m3y4-q-isgameover-meaning
kind: single
text: Зачем нужен флаг isGameOver?
answers:
  - text: Чтобы одним переключателем останавливать игровые действия при проигрыше
    correct: true
  - text: Чтобы ускорять физику после столкновения с препятствием и включать новый уровень
  - text: Чтобы хранить последнюю позицию игрока перед прыжком
```

---

## Пояснение (isGameOver)

- `isGameOver` помогает не запускать управление/спавн/счёт, когда игра окончена.

---

```quiz
id: l2m3y4-q-game-flow
kind: single
text: Какой порядок правильнее для обычной игры?
answers:
  - text: Start -> Play -> Game Over -> Restart
    correct: true
  - text: Restart -> Game Over -> Start -> Play
  - text: Play -> Start -> Restart -> Game Over
```

---

## Пояснение (игровой цикл)

- Обычно игра начинается, идёт, заканчивается, и потом игрок запускает заново.

---

## Практика 1: Мини-повторение (Quiz)

### Часть 1: Проверка знаний (Quiz)

```quiz
id: l2m3y4-p1-q-raycast-purpose
kind: single
text: Зачем мы делали Raycast вниз в DinoController?
answers:
  - text: Чтобы понять, на земле ли игрок (и разрешать прыжок)
    correct: true
  - text: Чтобы рисовать UI-подсказку рядом с игроком
  - text: Чтобы увеличивать скорость, когда игрок касается земли
```

---

## Ответы: Практика 1 (Quiz)

- Raycast вниз — чтобы получить `isGrounded`.

---

```quiz
id: l2m3y4-p1-q-and-vs-or
kind: single
text: Какой оператор означает "ИЛИ" (достаточно одного условия)?
answers:
  - text: &&
  - text: ||
    correct: true
  - text: ==
```

---

## Ответы: Практика 1 (Quiz) — продолжение

- `||` — логическое ИЛИ.

---

### Часть 2: Маленькая тренировка в коде

**Задача:**

1. Создайте скрипт `GameStateDemo.cs`.
2. Сделайте `bool isGameOver = false;`
3. В `Start()` выведите в консоль:
   - если `isGameOver` -> "Экран Game Over"
   - иначе -> "Играем"

```check
rules:
  - filename: "GameStateDemo.cs"
  - contains: "bool isGameOver"
  - contains: "if ("
  - contains: "else"
  - contains: "Debug.Log"
```

---

## Перерыв

---

UI — это то, что игрок видит: очки, время, кнопки.
Сейчас подключим TextMeshPro и сделаем экран Game Over.

---

## Теория 2: Перезапуск сцены и UI

Перезапуск игры чаще всего делают через перезагрузку текущей сцены:

```csharp
SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex);
```

Для этого нужен `using UnityEngine.SceneManagement;`.

UI обычно обновляют каждый кадр или по событию, например:

```csharp
scoreText.text = score.ToString();
```

Если счёт хранится как `float`, а на экране мы хотим видеть целое число без дробей, можно сделать так:

```csharp
scoreText.text = ((int)score).ToString();
```

Здесь:

- внутренние скобки `(int)score` — это **приведение типа**: мы говорим C#, что хотим превратить `score` из `float` в `int`;
- после этого получается уже целое число без дробной части;
- внешние скобки вокруг `((int)score)` помогают увидеть, что `.ToString()` вызывается уже **после** преобразования в `int`;
- `.ToString()` превращает получившееся число в текстовую строку.

Можно читать эту строку по шагам так:

1. взять `score`;
2. превратить его в `int`;
3. у результата вызвать `.ToString()`;
4. готовый текст положить в `scoreText.text`.

- `(int)score` отбрасывает дробную часть;
- `.ToString()` превращает число в текст, потому что UI-текст работает именно со строками.

---

```quiz
id: l2m3y4-q-restart-api
kind: single
text: Каким методом чаще всего перезапускают текущую сцену?
answers:
  - text: SceneManager.LoadScene(...)
    correct: true
  - text: SceneManager.RestartScene(...)
  - text: GameObject.ReloadScene(...)
```

---

## Пояснение (перезапуск)

- Перезапуск через `SceneManager.LoadScene` — самый простой и надёжный способ для курса.

---

```quiz
id: l2m3y4-q-ui-text-type
kind: single
text: Какой компонент чаще всего используют для текста в UI (в наших уроках)?
answers:
  - text: TMP_Text (TextMeshPro)
    correct: true
  - text: UnityEngine.UI.Text (Legacy)
  - text: Collider (Physics)
```

---

## Пояснение (TextMeshPro)

- Для текста мы используем TextMeshPro (`TMP_Text`).

---

## 💡 Лайфхак: один скрипт хранит правила

Когда появляется Game Over, легко раскидать проверки по всем объектам.
Но тогда игру сложно чинить.

Удобнее выбрать главный скрипт, например `GameController`, и хранить там:

- счёт;
- таймер;
- состояние `isGameOver`;
- метод перезапуска.

Остальные объекты только спрашивают у него: "игра ещё идёт?"

---

## Практика 2: Применение в игре — GameController + UI + Restart

### Шаг 1: UI на сцене

1. Создайте `Canvas`.
2. Добавьте текст `ScoreText` (TextMeshPro).
3. Создайте панель `GameOverPanel` и выключите её (`SetActive(false)`).
4. Добавьте на панель кнопку `RestartButton`.

```check
rules:
  - object_exists: "Canvas"
  - object_exists: "GameOverPanel"
  - object_exists: "RestartButton"
```

---

### Шаг 2: `GameController.cs` — счёт, таймер и Game Over

**Задача:**

1. Создайте `GameController.cs` и объект `GameController` на сцене.
2. Добавьте поля:
   - `public bool isGameOver;`
   - `public TMP_Text scoreText;`
   - `public GameObject gameOverPanel;`
   - `private float score;`
3. В `Update()` считайте очки (пока игра не окончена):
   - `score += Time.deltaTime;`
   - `scoreText.text = ((int)score).ToString();`
4. Добавьте `public void GameOver()`:
   - `isGameOver = true;`
   - `gameOverPanel.SetActive(true);`
5. Добавьте `public void Restart()`:
   - перезагрузить сцену

```csharp
using UnityEngine;
using UnityEngine.SceneManagement;
using TMPro;

public class GameController : MonoBehaviour
{
    public bool isGameOver;
    public TMP_Text scoreText;
    public GameObject gameOverPanel;

    private float score;

    void Update()
    {
        if (!isGameOver)
        {
            score += Time.deltaTime;
            scoreText.text = ((int)score).ToString();
        }
    }

    public void GameOver()
    {
        isGameOver = true;
        gameOverPanel.SetActive(true);
    }

    public void Restart()
    {
        SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex);
    }
}
```

> **Как работает GameController по шагам:**
>
> 1. Пока `isGameOver == false`, в `Update()` каждый кадр увеличивается `score` и обновляется текст `scoreText`.
> 2. Когда где‑то в игре вызывается `GameOver()`, флаг `isGameOver` становится `true`, а `gameOverPanel.SetActive(true)` показывает экран Game Over.
> 3. После этого код внутри `if (!isGameOver)` больше не выполняется — счёт перестаёт расти, игра «замораживается».
> 4. Метод `Restart()` перезагружает текущую сцену через `SceneManager.LoadScene`, чтобы начать игру заново.

```check
rules:
  - filename: "GameController.cs"
  - contains: "public bool isGameOver"
  - contains: "void GameOver()"
  - contains: "gameOverPanel.SetActive(true)"
  - contains: "void Restart()"
  - contains: "SceneManager.LoadScene"
  - contains: "score += Time.deltaTime"
  - contains: "scoreText.text"
```

---

Перед следующими шагами разберём ещё одну полезную команду:

```csharp
FindFirstObjectByType<GameController>()
```

Она ищет на сцене первый объект с компонентом `GameController` и возвращает ссылку на него.
Это удобно, когда другому скрипту нужно быстро найти главный контроллер игры.

---

### Шаг 3: Связь препятствия с Game Over

В `ObstacleTrigger.cs` (создан в уроке 3) вместо `Debug.Log` нужно вызывать `GameOver()` у контроллера.

```csharp
using UnityEngine;

public class ObstacleTrigger : MonoBehaviour
{
    void OnTriggerEnter(Collider other)
    {
        GameController gc = FindFirstObjectByType<GameController>();
        gc.GameOver();
    }
}
```

```check
rules:
  - filename: "ObstacleTrigger.cs"
  - contains: "GameOver"
  - contains: "FindFirstObjectByType"
```

---

### Шаг 4: Блокируем управление при Game Over

В `DinoController.cs` добавьте ссылку на `GameController` и в начале `Update()` проверяйте флаг:

```csharp
private GameController gc;

void Awake()
{
    rb = GetComponent<Rigidbody>();
    gc = FindFirstObjectByType<GameController>();
}

void Update()
{
    if (!gc.isGameOver)
    {
        // ... движение и прыжок как раньше ...
    }
}
```

```check
rules:
  - filename: "DinoController.cs"
  - contains: "isGameOver"
  - contains: "if ("
```

---

### Шаг 5: Останавливаем спавнер при Game Over

В `ObstacleSpawner.cs` тоже нужно прекратить создавать препятствия при проигрыше:

```csharp
private GameController gc;

void Start()
{
    gc = FindFirstObjectByType<GameController>();
}

void Update()
{
    if (!gc.isGameOver)
    {
        timer += Time.deltaTime;
        // ... Instantiate как раньше ...
    }
}
```

```check
rules:
  - filename: "ObstacleSpawner.cs"
  - contains: "isGameOver"
```

---

### Шаг 6: Кнопка Restart

В инспекторе у `RestartButton` в `OnClick` привяжите метод `GameController.Restart()`.

`OnClick` у кнопки — это список действий, которые Unity выполняет при нажатии.
То есть здесь мы буквально говорим кнопке: "когда на тебя нажмут, вызови метод `Restart()` у `GameController`".

---

## Итоги

На уроке мы:

- добавили состояние игры (`isGameOver`) и научились "останавливать" логику
- сделали подсчёт очков (таймер) и вывод в UI через TextMeshPro
- связали столкновение с препятствием и Game Over
- заблокировали управление и спавнер при проигрыше
- добавили перезапуск сцены через `SceneManager.LoadScene`
- завершили Dino 3D и закрепили темы модуля 3
