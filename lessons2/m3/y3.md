...
## Практика 2: Применение в игре — спавнер препятствий (заготовка)

На этом уроке мы делаем основу “бесконечности”: препятствия будут появляться сами.

### Шаг 1: Подготовка префаба препятствия

1. Создайте `Obstacle` (Cube/любой примитив).
2. Добавьте `Collider`, включите `Is Trigger`.
3. Сделайте `Prefab` `Obstacle.prefab`.

```check
rules:
  - filename: "Obstacle.prefab"
```
...
**Задача (заготовка, допишем на уроке):**

1. Создайте `ObstacleSpawner.cs`.
2. Добавьте поля:
...
- начали бесконечный Dino 3D: подготовили основу спавна препятствий (заготовка)
