...
## Теория 3: Trigger, Prefab и создание объектов из кода

Перед игровой практикой разберём ещё несколько Unity-вещей:

- `Is Trigger` у коллайдера означает: объект не толкает физически, а просто сообщает о пересечении;
- `OnTriggerEnter(Collider other)` — метод Unity, который вызывается при входе другого объекта в trigger;
- `Prefab` — шаблон объекта, из которого можно создавать копии;
- `Instantiate(...)` — создать копию prefab в сцене;
- `Destroy(...)` — удалить объект из сцены;
- `Quaternion.identity` — “обычный поворот”, то есть без дополнительного вращения.

Эти команды как раз нужны, чтобы препятствия можно было один раз настроить, а потом создавать снова и снова.

---

## Практика 2: Применение в игре — препятствие и столкновение
...
## Практика 3: Применение в игре — спавнер препятствий

Теперь отдельно сделаем спавнер, который будет создавать новые копии препятствий через время.

---

### Шаг 1: Скрипт `ObstacleSpawner.cs` (таймер + повторение)
...
### Шаг 2 (усложнение): "пачка препятствий" через `for`
