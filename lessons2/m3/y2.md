...
answers:
  - text: Все ветки выполняются по очереди, если условия написаны правильно
...
answers:
  - text: Только одна — первая подходящая
    correct: true
  - text: Две подходящие ветки подряд, если оба условия true
  - text: Все ветки до блока else
```
...
> **Как читать это условие для прыжка:**
>
> 1. `isGrounded` — стоим ли мы на земле?
> 2. `jumpPressed` — нажата ли кнопка прыжка?
> 3. `!isPaused` — **НЕ** на паузе?
> 4. Прыжок сработает только если **все три** условия `true`.
...
Перед шагами разберём ещё три маленькие вещи:

- `Input.GetMouseButtonDown(0)` — проверка нажатия левой кнопки мыши;
- число `0` здесь означает именно ЛКМ;
- `offset` — это смещение камеры относительно игрока по оси X, то есть “насколько в стороне” она должна держаться.

Для камеры также удобно использовать `LateUpdate()`, потому что этот метод вызывается после `Update()`.
Значит, сначала игрок успевает сдвинуться, а потом камера его “догоняет”.
